...
import { Eye, User, Laptop, Key, Download, Search, FileText, Printer, Upload, AlertCircle } from 'lucide-react';
...
export const AssignmentsTab: React.FC<AssignmentsTabProps> = ({ usuarios, equipos }) => {
  ...
  const ITEMS_PER_PAGE = 5; // Cards are large, show fewer
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Filtrar solo asignaciones vigentes (fecha_fin es null o undefined) del historial
      setActiveAssignments(allAssignments.filter(a => !a.fecha_fin));
  ...
  const handleUploadClick = (id: number) => {
      if (id === -1) {
          Swal.fire({
              ...
              text: 'Este equipo figura asignado en inventario pero no tiene un registro de movimiento histórico activo (posible dato migrado). Realice una re-asignación para regularizar y subir documentos.',
              icon: 'warning',
  ...
  const allUserAssetsList: UserAssets[] = usuarios.map(user => {
    // 1. Obtener equipos del INVENTARIO ACTUAL (Fuente de Verdad)
    const currentInventory = equipos.filter(e => e.responsable_id === user.id);

    // 2. Mapear inventario a estructura de visualización, buscando coincidencia en historial
    const userEq = currentInventory.map(invItem => {
        // Buscar si existe un historial activo para este equipo
        const historyMatch = activeAssignments.find(a => a.equipo_codigo === invItem.codigo_activo);

        if (historyMatch) {
            return historyMatch;
        } else {
            // FALLBACK: Si está asignado en inventario pero no hay historial (ej. migración)
            // Creamos un objeto dummy para mostrarlo en el reporte
            return {
                id: -1, // ID negativo indica que no es un registro de base de datos editable
                equipo_codigo: invItem.codigo_activo,
                equipo_modelo: `${invItem.marca} ${invItem.modelo}`,
                usuario_nombre: user.nombre_completo,
                usuario_departamento: user.departamento_nombre || '',
                fecha_inicio: 'Fecha Desconocida', // Opcional: Podría usarse fecha_compra o similar
                fecha_fin: null,
                ubicacion: invItem.ubicacion_nombre || 'Ubicación Usuario',
                archivo_pdf: undefined
            } as HistorialAsignacion;
        }
    });
    ...
    // Ocultar usuarios sin activos
    return item.equipment.length > 0 || item.licenses.length > 0;
  });

  // Pagination Logic
  const totalPages = Math.ceil(allUserAssetsList.length / ITEMS_PER_PAGE);
  const paginatedUserAssets = allUserAssetsList.slice(
      (currentPage - 1) * ITEMS_PER_PAGE,
      currentPage * ITEMS_PER_PAGE
  );

  const prepareExportData = () => {
    return allUserAssetsList.flatMap(item => {
  ...
  const handlePrintPDF = () => {
      let htmlContent = `<style>.user-card { border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 25px; overflow: hidden; page-break-inside: avoid; } .user-header { background-color: #f8fafc; padding: 12px 16px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; } .avatar { width: 32px; height: 32px; background-color: #2563eb; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 12px; margin-right: 10px; } .user-info { flex: 1; } .user-name { font-size: 15px; font-weight: 700; color: #1e293b; } .user-meta { font-size: 11px; color: #64748b; margin-top: 2px; } .counts { display: flex; gap: 8px; } .count-badge { padding: 4px 10px; border-radius: 99px; font-size: 10px; font-weight: 600; border: 1px solid transparent; } .count-eq { background-color: #eff6ff; color: #1d4ed8; border-color: #dbeafe; } .count-lic { background-color: #faf5ff; color: #7e22ce; border-color: #f3e8ff; } .grid-container { display: flex; border-top: 1px solid #e2e8f0; } .col-section { flex: 1; border-right: 1px solid #e2e8f0; } .col-section:last-child { border-right: none; } .section-header { padding: 8px 12px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; display: flex; align-items: center; gap: 6px; } .header-eq { background-color: #eff6ff; color: #1e40af; border-bottom: 1px solid #dbeafe; } .header-lic { background-color: #faf5ff; color: #6b21a8; border-bottom: 1px solid #f3e8ff; } .empty-state { padding: 15px; text-align: center; color: #cbd5e1; font-style: italic; font-size: 11px; } table.inner-table { width: 100%; border-collapse: collapse; } table.inner-table th { background: transparent; border-bottom: 1px solid #f1f5f9; padding: 6px 12px; font-size: 10px; text-align: left; color: #64748b; font-weight: 600; text-transform: uppercase; } table.inner-table td { padding: 6px 12px; font-size: 11px; border-bottom: 1px solid #f8fafc; color: #334155; } table.inner-table tr:last-child td { border-bottom: none; }</style>`;
      if(allUserAssetsList.length === 0) htmlContent += `<div style="text-align:center;">No data</div>`;
      allUserAssetsList.forEach(({ user, equipment, licenses }) => {
          htmlContent += `<div class="user-card"><div class="user-header"><div style="display:flex; align-items:center;"><div class="avatar">${user.nombres.charAt(0)}${user.apellidos.charAt(0)}</div><div class="user-info"><div class="user-name">${user.nombre_completo}</div><div class="user-meta">${user.departamento_nombre || 'Sin Depto'}</div></div></div><div class="counts"><span class="count-badge count-eq">Equipos: ${equipment.length}</span></div></div><div class="grid-container"><div class="col-section"><div class="section-header header-eq">Equipos</div>${equipment.length > 0 ? `<table class="inner-table"><thead><tr><th>Código</th><th>Modelo</th></tr></thead><tbody>${equipment.map(e => `<tr><td>${e.equipo_codigo}</td><td>${e.equipo_modelo}</td></tr>`).join('')}</tbody></table>` : `<div class="empty-state">Sin equipos</div>`}</div></div></div>`;
      });
      ...
      <div className="space-y-8">
        {loading ? (
             ...
             <div className="p-12 text-center text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800">
                No se encontraron asignaciones activas para los criterios seleccionados.
             </div>
        ) : (
            <>
                {paginatedUserAssets.map(({ user, equipment, licenses }) => (
                                ...
                                {equipment.length > 0 ? (
                                    <table className="min-w-full text-sm">
                                        ...
                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                            {equipment.map(eq => (
                                                <tr key={eq.id === -1 ? `dummy-${eq.equipo_codigo}` : eq.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                                    ...
                                                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">
                                                        {eq.id === -1 ? (
                                                            <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400 text-xs" title="Sin registro histórico">
                                                                <AlertCircle className="w-3 h-3" /> {eq.fecha_inicio}
                                                            </span>
                                                        ) : (
                                                            eq.fecha_inicio
                                                        )}
                                                    ...
                                                    <td className="px-4 py-3 text-center">
                                                        {eq.archivo_pdf ? (
                                                            ...
                                                            <button onClick={() => handleUploadClick(eq.id)} className={`p-1 rounded ${eq.id === -1 ? 'text-slate-300 dark:text-slate-600 cursor-not-allowed' : 'text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`} title={eq.id === -1 ? "Regularizar asignación para subir" : "Subir Documento Firmado"}>
                ...
                <Pagination 
                    ...
                    totalItems={allUserAssetsList.length}
