    ...
    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 space-y-4">
      ...
      <div className="flex items-center justify-end pt-2 border-t border-slate-100">
        ...
        <div className="flex items-center border rounded-lg overflow-hidden bg-white">
          <button onClick={() => setGrouping('NONE')} className={`px-3 py-1.5 text-xs font-medium flex items-center gap-2 ${grouping === 'NONE' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
            <Layers className="w-3 h-3" /> Plano
          </button>
          <div className="w-px h-6 bg-slate-200"></div>
          <button onClick={() => setGrouping('TYPE')} className={`px-3 py-1.5 text-xs font-medium flex items-center gap-2 ${grouping === 'TYPE' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
            <Box className="w-3 h-3" /> Tipo
          </button>
          <div className="w-px h-6 bg-slate-200"></div>
          <button onClick={() => setGrouping('USER')} className={`px-3 py-1.5 text-xs font-medium flex items-center gap-2 ${grouping === 'USER' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
